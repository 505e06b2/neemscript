...
class Neem {
	...
	private:
		enum types {none_, comment_, echo_, set_, prompt_, setsystem_, import_, goto_, call_, inc_, fi_, if_, else_, switch_, case_, for_, rof_,
					exit_, label_, sleep_, start_, pwd_, cd_, rm_, rmdir_, ls_, pause_, output_, input_, readall_, readline_, startlocal_, endlocal_,
		...
		int command_startlocal(instruction *, uint32_t);
